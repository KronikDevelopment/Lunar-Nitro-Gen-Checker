...
os.system("cls")
shitter =  Fore.WHITE + "Coded with love"
heart =  Fore.RED + "<3"
kronik = Fore.WHITE + "kronik luvs u"
USE_WEBHOOK = True
print("")
print(shitter,heart)
print(kronik)
...
class NitroGen:  # Initialise the class
    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Lunar - Made by kronik#0001 | discord.gg/kronik")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by kronik#0001\a',
 ...
 |______|  \__,_| |_| |_|  \__,_| |_|   
        ...
        print(".gg/kronik")
        print("")
        self.slowType( Fore.CYAN + "Made by: kronik#0001", .02)
        self.slowType( Fore.RED +"This is free so if you paid you got scammed!", .02)
        time.sleep(1)  # Wait a little more
        self.slowType( Fore.CYAN +
        ...
        try:
            ...
            input( Fore.RED + "Specified input wasn't a number.\nPress enter to exit")
            exit()  # Exit program
        ...
        if USE_WEBHOOK:
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Lunar | Made By kronik#0001\n\nStarted checking urls\nI will send any valid codes here```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            try:
                ...
                print( Fore.RED + f" Error | {url} ")  # Tell the user an error occurred

            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Lunar - {len(valid)} Valid | {invalid} Invalid - Made by kronik#0001")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Lunar - {len(valid)} Valid | {invalid} Invalid - Made by kronik#0001\a', end='', flush=True)
 ...
 Valid Codes: {', '.join(valid)}""")  # Give a report of the results of the check
    ...
    def quickChecker(self, nitro:str, notify=None):  # Used to check a single code at a time
        ...
        if response.status_code == 200:  # If the responce went through
            ...
            if notify is not None:  # If a webhook has been added
                DiscordWebhook(  # Send the message to discord letting the user know there has been a valid nitro code
                    url=url,
                    content=f"NIGGERRR WAKE UP A VALID NITRO CODE DETECTED! @everyone \n{nitro}"
                ).execute()
        ...
        # If the responce got ignored or is invalid ( such as a 404 or 405 )
            ...
            print( Fore.RED + f" Invalid | {nitro} ", flush=True,
                  end="" if os.name == 'nt' else "\n")
